...
ShadowFbo::~ShadowFbo()
{
	...
	if (sm_Texture != 0)
		glDeleteTextures(1, &sm_Texture);
	if (textureNormalized != 0)
		glDeleteTextures(1, &textureNormalized);
	if (blurredTexture != 0)
		glDeleteTextures(1, &blurredTexture);
}


void ShadowFbo::CreateFBO(const int w, const int h)
{
	glGenFramebuffers(1, &shadowFbo); //create shadowmap fbo
	glBindFramebuffer(GL_FRAMEBUFFER, shadowFbo);	// set shadowmap fbo to current fbo


	// normalized depth buffer texture, store depth in R component
	glGenTextures(1, &textureNormalized);
...
void ShadowFbo::BindForWrite()
{
	glBindFramebuffer(GL_DRAW_FRAMEBUFFER, shadowFbo);
	CHECKERROR
}

void ShadowFbo::BindForRead(GLenum textureUnit, int smStage)
{
...
void ShadowFbo::BindBlurredForRead(GLenum textureUnit)
{
	glActiveTexture(textureUnit);
	glBindTexture(GL_TEXTURE_2D, blurredTexture);
	CHECKERROR
