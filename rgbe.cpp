...
static int rgbe_error(int rgbe_error_code, const char *msg, char *errbuf)
{
  ...
  case rgbe_read_error:
      if (errbuf)
          strcpy_s(errbuf, sizeof(errbuf),"RGBE read error");
      else
          perror("RGBE read error");
    break;
  case rgbe_write_error:
      if (errbuf)
          strcpy_s (errbuf, sizeof(errbuf), "RGBE write error");
      else
          perror("RGBE write error");
    break;
  case rgbe_format_error:
      if (errbuf)
          sprintf_s(errbuf, sizeof(errbuf), "RGBE bad file format: %s\n", msg);
      else
  ...
  case rgbe_memory_error:
      if (errbuf)
          sprintf_s(errbuf,sizeof(errbuf),"RGBE error: %s\n",msg);
      else
